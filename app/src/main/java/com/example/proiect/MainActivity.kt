...
class WeatherRepo(private val favoritesDao: FavoritesDao) {
    ...
    suspend fun fetchWeatherData(lat: String, lon: String): WeatherData? = withContext(Dispatchers.IO) {
        val url = "https://api.open-meteo.com/v1/forecast?latitude=$lat&longitude=$lon&current=temperature_2m,weather_code&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=auto"
        try {
...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val cityToFetch = city ?: (activity as MainActivity).currentCity ?: City("44.43,26.10", "București", 44.43, 26.10)
...
class ForecastAdapter(context: Context, private val forecasts: List<DailyForecast>, private val repo: WeatherRepo) :
    ArrayAdapter<DailyForecast>(context, 0, forecasts) {
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        val view = convertView ?: LayoutInflater.from(context).inflate(R.layout.item_forecast, parent, false)
        val forecast = getItem(position)
        val tvDate = view.findViewById<TextView>(R.id.tvDate)
        val tvCondition = view.findViewById<TextView>(R.id.tvCondition)
        val tvTemps = view.findViewById<TextView>(R.id.tvTemps)

        forecast?.let {
            tvDate.text = it.date
            tvCondition.text = repo.getWeatherDescription(it.weatherCode)
            tvTemps.text = "${it.maxTemp}° / ${it.minTemp}°"
        }
        return view
    }
}
...
class ForecastFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, c: ViewGroup?, state: Bundle?) = inflater.inflate(R.layout.fragment_forecast, c, false)

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        val lvDaily = view.findViewById<ListView>(R.id.lv_daily_forecast)
        val cityToFetch = city ?: (activity as MainActivity).currentCity ?: City("44.43,26.10", "București", 44.43, 26.10)

        lifecycleScope.launch {
            val data = repo.fetchWeatherData(cityToFetch.lat.toString(), cityToFetch.lon.toString())
            data?.let {
                val adapter = ForecastAdapter(requireContext(), it.daily, repo)
                lvDaily.adapter = adapter
            }
        }
    }
    
    companion object {
        fun newInstance(city: City) = ForecastFragment().apply {
            arguments = Bundle().apply {
                putString("id", city.id); putString("name", city.name)
                putDouble("lat", city.lat); putDouble("lon", city.lon)
            }
...
class FavoritesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lvFavs.setOnItemClickListener { _, _, position, _ ->
            (activity as? MainActivity)?.changeCurrentCity(favoriteCities[position])
        }
...
class SettingsFragment : Fragment() {
    override fun onCreateView(inflater: LayoutInflater, c: ViewGroup?, state: Bundle?) = inflater.inflate(R.layout.fragment_settings, c, false)
}
...
class MainActivity : AppCompatActivity() {
    ...
    private val locationPermissionRequest = registerForActivityResult(
        ...
        if (isGranted) {
            fetchAndSetCurrentLocation()
             ...
             if (currentCity == null) {
                 updateContent(City("44.43,26.10", "București", 44.43, 26.10))
             }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bottomNav.setOnItemSelectedListener { 
            when (it.itemId) {
                R.id.nav_home -> {
                    val fragment = currentCity?.let { city -> HomeFragment.newInstance(city) } ?: HomeFragment()
                    supportFragmentManager.beginTransaction().replace(R.id.fragment_container, fragment).commit()
                }
                R.id.nav_forecast -> {
                    val fragment = currentCity?.let { city -> ForecastFragment.newInstance(city) } ?: ForecastFragment()
                    supportFragmentManager.beginTransaction().replace(R.id.fragment_container, fragment).commit()
                }
                R.id.nav_favorites -> {
    ...
    private fun fetchAndSetCurrentLocation() {
    ...
    fun changeCurrentCity(city: City) {
        ...
        val fragment = HomeFragment.newInstance(city)
        supportFragmentManager.beginTransaction().replace(R.id.fragment_container, fragment).commit()
    }

    private fun updateContent(city: City) {
        currentCity = city
        val fragment = HomeFragment.newInstance(city)
        supportFragmentManager.beginTransaction().replace(R.id.fragment_container, fragment).commit()
    }
